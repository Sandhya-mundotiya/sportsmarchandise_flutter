...
import 'package:merch/bloc/add_product/add_product_bloc.dart' as add_product_bloc;
import 'package:merch/bloc/edit_product/edit_product_bloc.dart' as edit_product_bloc;
...
class ProductRepository extends BaseProductRepository {
  ...
  addProduct({Product productObj,BuildContext context,List<Asset> assetImages}) {
    ...
    uploadFiles(assetImages).then((value){
      addProductToFireStore(productObj: productObj,context: context,images: value);
    });
  ...
  updateProduct({Product productObj,BuildContext context,List<Asset> assetImages}) {
    ...
    if(assetImages != null && assetImages.length > 0){
      uploadFiles(assetImages).then((value){
        uploadProductToFireStore(context: context,images: value,productObj: productObj);
      });
    }else{
      uploadProductToFireStore(context: context,images: [],productObj: productObj);
    }
  ...
  deleteImage({String image, BuildContext context,String uid}) async{
    var photoRef = await FirebaseStorage.instance.ref().storage.refFromURL(image);
    try {
    ...
    _firebaseFirestore.collection(SCHOOL_TABLE).doc(SchoolData.schoolId).collection(PRODUCT_TABLE).doc(uid).update(
      {
        ...
        .then((value){
      context.read<edit_product_bloc.EditProductBloc>().add(edit_product_bloc.SuccessfulyDeletedImage(deletedImageUrl: image));
  ...
  addProductToFireStore({Product productObj,BuildContext context,List<String> images}){
    ...
    reference.add(newProduct.toJson()).then((value)  {
      ...
      context.read<add_product_bloc.AddProductBloc>().add(add_product_bloc.StopLoading());
      Navigator.pop(context);
  ...
  uploadProductToFireStore({Product productObj,BuildContext context,List<String> images}){
    _firebaseFirestore.collection(SCHOOL_TABLE).doc(SchoolData.schoolId).collection(PRODUCT_TABLE).doc(productObj.uid).update(
      {
        ...
        if(images != null && images.length > 0) 'images': FieldValue.arrayUnion(images)
      },
    )
        .then((value){
      context.read<edit_product_bloc.EditProductBloc>().add(edit_product_bloc.StopLoading());
      Navigator.pop(context);
